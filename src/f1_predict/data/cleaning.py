...
class DataCleaner:
    ...
    def clean_race_results(
        ...
        if self.enable_logging:
            self.logger.info(
                f"Cleaned {len(cleaned_data)}/{len(results_data)} results (quality: {report.quality_score:.1f}%)"
            )

        return cleaned_data, report

    def clean_qualifying_results(
        self, qualifying_data: list[dict[str, Any]]
    ) -> tuple[list[dict[str, Any]], DataQualityReport]:
        ...
        for result in qualifying_data:
            try:
                ...
                report.validation_errors.append(
                    f"Error cleaning qualifying result: {str(e)}"
                )
                if self.enable_logging:
        ...
        if self.enable_logging:
            self.logger.info(
                f"Cleaned {len(cleaned_data)}/{len(qualifying_data)} qualifying results (quality: {report.quality_score:.1f}%)"
            )

        return cleaned_data, report

    def clean_race_schedules(
        ...
        if self.enable_logging:
            self.logger.info(
                f"Cleaned {len(cleaned_data)}/{len(schedule_data)} schedules (quality: {report.quality_score:.1f}%)"
            )
    ...
    def _clean_race_result(
        self, result: dict[str, Any], report: DataQualityReport
    ...
    def _clean_race_schedule(
        self, schedule: dict[str, Any], report: DataQualityReport
    ) -> Optional[dict[str, Any]]:
    ...
    def _handle_missing_values(
        self, data: dict[str, Any], report: DataQualityReport, data_type: str
    ) -> dict[str, Any]:
        ...
        for key, value in data.items():
            if (
                ...
                report.missing_values[field_key] = (
                    report.missing_values.get(field_key, 0) + 1
                )
    ...
    def _standardize_driver_name(
        self, data: dict[str, Any], report: DataQualityReport
    ) -> dict[str, Any]:
    ...
    def _standardize_constructor_name(
        self, data: dict[str, Any], report: DataQualityReport
    ) -> dict[str, Any]:
    ...
    def _convert_qualifying_result_types(
        self, data: dict[str, Any], report: DataQualityReport
    ) -> dict[str, Any]:
    ...
    def _convert_to_numeric(
        self, value: Any, field: str, report: DataQualityReport
    ...
    def _normalize_lap_time(
        self, time_str: str, report: DataQualityReport
    ) -> Optional[str]:
    ...
    def _validate_qualifying_result(
        self, data: dict[str, Any], report: DataQualityReport
    ) -> bool:
        ...
        if data.get("position"):
            if not (1 <= data["position"] <= 30):
                report.validation_errors.append(
                    f"Invalid qualifying position: {data['position']}"
                )
                return False

        return True

    def _validate_race_schedule(
        self, data: dict[str, Any], report: DataQualityReport
    ) -> bool:
        ...
        if data.get("longitude") is not None:
            if not (-180 <= data["longitude"] <= 180):
                report.validation_errors.append(
                    f"Invalid longitude: {data['longitude']}"
                )
                return False
    ...
    def _calculate_quality_score(self, report: DataQualityReport) -> float:
        ...
        type_issues_percent = (
            len(report.data_type_issues) / report.total_records
        ) * 100
        score -= min(
            30, type_issues_percent * 10
        )  # 10 points per percent with type issues

        # Deduct for validation errors (max 40 points)
        validation_errors_percent = (
            len(report.validation_errors) / report.total_records
        ) * 100
        score -= min(
            40, validation_errors_percent * 20
        )  # 20 points per percent with errors

        # Add points for standardization (up to 10 points)
        total_standardizations = sum(
            len(changes) for changes in report.standardization_changes.values()
        )
...
class DataQualityValidator:
    ...
    def _check_duplicates(
        self, data: list[dict[str, Any]], report: DataQualityReport, data_type: str
    ):
        """Check for duplicate records."""
        if data_type == "race_results":
            ...
            for item in data:
                ...
                if key in seen:
                    report.validation_errors.append(
                        f"Duplicate qualifying result: {key}"
                    )
                seen.add(key)
        ...
        if data_type == "race_results":
            ...
            for race_key, positions in races.items():
                if len(set(positions)) != len(positions):
                    report.validation_errors.append(
                        f"Duplicate positions in race {race_key}"
                    )
    ...
    def _calculate_dataset_quality_score(
        self, data: list[dict[str, Any]], report: DataQualityReport
    ) -> float:
        ...
        total_missing = sum(report.missing_values.values())
        missing_rate = total_missing / (
            total_records * 10
        )  # Assume avg 10 fields per record
